...
export function App() {
	...
	const saveProfiles = async (newProfiles: Record<string, Profile>, newSelectedId: string | null) => {
	...
	return (
		<div style={{ display: "flex", height: "100vh", fontFamily: "system-ui" }}>
			{/* Sidebar */}
			<div style={{ width: "300px", borderRight: "1px solid #ccc", padding: "20px", backgroundColor: "#f9f9f9" }}>
				<h2>Profiles</h2>
				<button onClick={createProfile} style={{ width: "100%", padding: "10px", marginBottom: "20px" }}>
					+ Create Profile
				</button>
				...
				{editingProfile ? (
					<div>
						<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
							<h1>Edit Profile</h1>
							<div>
								<button onClick={() => deleteProfile(editingProfile.id)} style={{ marginRight: "10px", color: "red" }}>Delete Profile</button>
								<button onClick={saveEditingProfile} style={{ padding: "8px 20px", fontWeight: "bold" }}>Save Changes</button>
							</div>
						</div>
						...
						<section style={{ marginBottom: "30px" }}>
							...
							/>
							<br /><br />
							<label>Enabled Domains (comma separated, use * for all):</label><br />
							<input 
								type="text" 
								value={editingProfile.enabledDomains.join(", ")} 
								onChange={e => setEditingProfile({...editingProfile, enabledDomains: e.target.value.split(",").map(s => s.trim())})}
								style={{ width: "100%", padding: "8px", marginTop: "5px" }}
							/>
						</section>
						...
						<section>
							...
							<table style={{ width: "100%", borderCollapse: "collapse" }}>
								...
								<tbody>
									{Object.entries(editingProfile.mappings).map(([key, mapping]) => (
										<tr key={key} style={{ borderBottom: "1px solid #eee" }}>
											<td style={{ padding: "10px" }}>
												<input 
													...
													onChange={e => updateMapping(key, "keywords", e.target.value.split(",").map(s => s.trim()))}
											...
											<td style={{ padding: "10px" }}>
												<textarea 
													...
													onChange={e => updateMapping(key, "content", e.target.value)}
													style={{ width: "90%", padding: "5px", height: "40px" }}
												/>
											</td>
											<td style={{ padding: "10px" }}>
												<button onClick={() => deleteMapping(key)}>Remove</button>
											</td>
										</tr>
									))}
								</tbody>
